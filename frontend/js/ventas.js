...
async function cargarDashboard() {
  const productos = await fetch("http://localhost:8080/productos").then((r) =>
    r.json(),
  );
  const clientes = await fetch("http://localhost:8080/clientes").then((r) =>
    r.json(),
  );
  const ventas = await fetch("http://localhost:8080/ventas").then((r) =>
    r.json(),
  );
